...
export default function IdeasPage() {
  const boardIdeas = APP_MODE === "demo" ? ideas : [];
  const grouped = boardColumns
    .map((status) => ({
      status,
      items: boardIdeas.filter((idea) => idea.status === status),
    }))
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        {APP_MODE === "live" && (
          <div className="card p-8 text-center">
            <h2 className="text-xl text-heading">No live ideas yet</h2>
            <p className="mt-2 text-sm text-ink-soft">The live board starts clean. Demo ideas are not shown in this build.</p>
          </div>
        )}
